...
- `Model/` — DNS domain types:
  ...
  - `DnsRecordBase` (abstract) → `DnsRecord` (abstract) → `DnsRecord<T>` (abstract generic)
    ...
    - `DnsResponsiblePersonRecord`, `DnsAfsDatabaseRecord`, `DnsRouteThroughRecord` (sealed) — RP, AFSDB, and RT experimental typed records
  ...
  - `DnsName` (sealed) — immutable linked list of `DnsLabel` nodes. Dots always separate labels; escaping and Unicode are unsupported. `Length` is presentation characters including the trailing dot; `MaxLength` is 254, corresponding to the 255-byte wire limit. Parsing rejects interior empty labels. The constructor preserves the supplied parent; use `DnsName.Empty` to terminate non-root names
  - `DnsLabel` (readonly struct) — stores a string-backed `ReadOnlyMemory<char>` slice, reusing input strings (max 63 ASCII characters/bytes). Supports printable ASCII except dots and backslashes; spaces, controls, and non-ASCII are rejected in parsing and wire decoding. `Span` exposes label text. Equality/hash and compression matching ignore ASCII case; a default label equals `DnsLabel.Empty` and has the same hash code. Both labels and names expose optional `IsHostName`/`ParseHostName` ASCII hostname validation. IDNA conversion is explicit; asterisks are literal, not pattern matching
