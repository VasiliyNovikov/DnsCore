...
- `Model/` — DNS domain types:
  ...
  - `DnsLabel` (readonly struct) — stores a single `StringSegment`, reusing input segments (max 63 ASCII characters/bytes). Supports printable ASCII except dots and backslashes; spaces, controls, and non-ASCII are rejected in parsing and wire decoding. `Span` exposes label text. Equality/hash and compression matching ignore ASCII case. Both labels and names expose optional `IsHostName`/`ParseHostName` ASCII hostname validation. IDNA conversion is explicit; asterisks are literal, not pattern matching
